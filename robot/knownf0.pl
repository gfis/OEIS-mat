...
# Derive simple transformations from known functions with A-number argument
# @(#) $Id$
# 2023-09-26, Georg Fischer
#
#:# Usage:
#:#   perl knownf0.pl input.seq4 > output.seq4
#--------------------------------------------------------
use strict;
use integer;
use warnings;
...
my $line;
my ($aseqno, $callcode, $offset, $name, $rseqno, $fname);
my $headers = <DATA>;
...
while (<>) {
    ...
    #                  1           12 (       (2        1      12       2
    while($name =~ m{\b([a-zB-Z]\w*)(\(A\d{6}\()}) { # "function(A012345("
        my ($orgfun, $organ) = ($1, $2);
        $fname = lc($orgfun);
        $rseqno = $fmap{$fname};
        if (0) {
        } elsif (defined($rseqno)) {
            $name =~ s{$orgfun$organ}{$rseqno$organ};
        } elsif ($fname =~ m{abs|floor|round|ceiling|gcd|lcm|a||max|min|binomial|stirling\d?|num(erator)?|den(ominator)?|sum}) {
        	$rseqno = uc($fname);
            $name =~ s{$orgfun$organ}{$rseqno\!$organ};
        } else {
        	$rseqno = uc($fname);
    ...
    if ($name !~ m{[\?\#]}) {
        ...
        print STDERR join("\t", ($aseqno, "unknown", $offset, $fname, $name)) . "\n";
    }
} # while <>
#--------------------------------------------
# The following list is taken from known.pl.
__DATA__
fname	rseqno
pi,primepi	A000720	knowna	78	Puma.primePiZ(v)		pi(n), the number of primes <= n. Sometimes called PrimePi(n) to distinguish it from the number 3.14159...
numbpart	A000041	knowna	55	IntegerPartition.partitions(v.intValueExact())	a(n) is the number of partitions of n (the partition numbers).
prime	A000040	knowna	54	Puma.primeZ(v.intValueExact())	The prime numbers.
sigma0,numdiv,d,sigma_0	A000005	knowna	47	Jaguar.factor(v).sigma0()	d(n) (also called tau(n) or sigma_0(n)), the number of divisors of n.
...
bell	A000110	knowna	32	Bell.bell(v.intValueExact())	Bell or exponential numbers: number of ways to partition a set of n labeled elements.
triangular	A000217	knowna	29	v.multiply(v.add(1)).divide(2)	Triangular numbers: a(n) = binomial(n+1,2) = n*(n+1)/2 = 0 + 1 + 2 + ... + n.
oddpart	A000265	knowna	29	v.makeOdd()	Remove all factors of 2 from n; or largest odd divisor of n; or odd part of n.
omega	A001221	knowna	28	Z.valueOf(Jaguar.factor(v).omega())	Number of distinct primes dividing n (also called omega(n)).
	A064989	knowna	28		Multiplicative with a(2^e) = 1 and a(p^e) = prevprime(p)^e for odd primes p.
gpf	A006530	knowna	27	Jaguar.factor(v).largestPrimeFactor()	 greatest prime dividing n, for n >= 2; a(1)=1.
numdigits	A055642	knowna	27	Z.valueOf(v.toString().length())	Number of digits in the decimal expansion of n.
bigomega	A001222	knowna	25	Z.valueOf(Jaguar.factor(v).bigOmega())	Number of prime divisors of n counted with multiplicity (also called big omega of n, bigomega(n) or Omega(n)).
sigma,sumdiv,sigma_1	A000203	knowna	24	Jaguar.factor(v).sigma()	a(n) = sigma(n), the sum of the divisors of n. Also called sigma_1(n).
phi,tau	A000010	knowna	23	Euler.phi(v)	Euler totient function phi(n): count numbers <= n and prime to n.
...
catalan	A000108	knowna	19	{ final int n = v.intValueExact(); return Binomial.binomial(2*n, n).divide(n + 1); }	Catalan numbers: C(n) = binomial(2n,n)/(n+1) = (2n)!/(n!(n+1)!).
...
lpf	A020639	knowna	19	Jaguar.factor(v).leastPrimeFactor()	Lpf(n): least prime dividing n (when n > 1); a(1) = 1. Or, smallest prime factor of n, or smallest prime divisor of n.
primorial	A002110	knowna	17	ZUtils.primorial(v.longValueExact())	Primorial numbers (first definition): product of first n primes. Sometimes written prime(n)#.
nextprime	A151800	knowna	17	Puma.primeZ(Puma.primePi(v) + 1)	Least prime > n (version 2 of the "next prime" function).
	A007504	knowna	16		Sum of the first n primes.
	A003961	knowna	15		Completely multiplicative with a(prime(k)) = prime(k+1).
	A007918	knowna	15	Puma.primeZ(Puma.primePi(v) + 1)	Least prime >= n (version 1 of the "next prime" function).
fibonacci,fib	A000045	knowna	14	Fibonacci.fibonacci(v.intValueExact())	Fibonacci numbers: F(n) = F(n-1) + F(n-2) with F(0) = 0 and F(1) = 1.
	A007949	knowna	14	Z.valueOf(ZUtils.valuation(v, Z.THREE))	Greatest k such that 3^k divides n. Or, 3-adic valuation of n	.
	A000001	knowna	13	Z.valueOf(GroupFactory.smallGroups(v.intValueExact()).size())	Number of groups of order n.
parity	A000035	knowna	13	v.testBit(0) ? Z.ONE : Z.ZERO	Period 2: repeat [0, 1]; a(n) = n mod 2; parity of n.
hammingweight,wt,bitcount	A000120	knowna	13	Z.valueOf(v.bitCount())	1''s-counting sequence: number of 1''s in binary expansion of n (or the binary weight of n).
rad,radical	A007947	knowna	13	Jaguar.factor(v).squareFreeKernel()	Largest squarefree number dividing n: the squarefree kernel of n, rad(n), radical of n.
digitalroot	A010888	knowna	13	{ final int n = v.intValueExact(); return n == 0 ? Z.ZERO : Z.valueOf(1 + (n+8) % 9); }	Digital root of n (repeatedly add the digits of n until a single digit is reached).
deriv	A003415	knowna	12		a(n) = n'' = arithmetic derivative of n: a(0) = a(1) = 0, a(prime) = 1, a(m*n) = m*a(n) + n*a(m).
...
sqrt,sqrtint	A000196	knowna	11	v.sqrt()	Integer part of square root of n. Or, number of positive squares <= n. Or, n appears 2n+1 times.
...
sopf	A008472	knowna	11	Jaguar.factor(v).sopf()	Sum of the distinct primes dividing n.
fibonacci,fib	A000032	knowna	10	Fibonacci.lucas(v.intValueExact())	Lucas numbers beginning at 2: L(n) = L(n-1) + L(n-2), L(0) = 2, L(1) = 1.
	A001358	knowna	10		Semiprimes (or biprimes): products of two primes.
reverse,r	A004086	knowna	10	ZUtils.reverse(v)	Read n backwards (referred to as R(n) in many sequences).
...
digitsum	A007953	knowna	10	Z.valueOf(ZUtils.digitSum(v))	Digital sum (i.e., sum of digits) of n; also called digsum(n).
...
charfun	A010051	knowna	8	v.isProbablePrime() ? Z.ONE : Z.ZERO	Characteristic function of primes: 1 if n is prime, else 0.
...
prevprime	A151799	knowna	8	Puma.primeZ(Puma.primePi(v) - 1)	Version 2 of the "previous prime" function: largest prime < n.
...
sopfr	A001414	knowna	7	Jaguar.factor(v).sopfr()	Integer log of n: sum of primes dividing n (with repetition). Also called sopfr(n).
	A001951	knowna	7		A Beatty sequence: a(n) = floor(n*sqrt(2)).
	A001952	knowna	7		A Beatty sequence: a(n) = floor(n*(2 + sqrt(2))).
lambda	A002322	knowna	7	Carmichael.lambda(v)	Reduced totient function psi(n): least k such that x^k == 1 (mod n) for all x prime to n; also known as the Carmichael lambda function (exponent of unit group mod n); also called the universal exponent of n.
ord	A002326	knowna	7	IntegersMod(v).ord(Z.TWO.mod(v))	Multiplicative order of 2 mod 2n+1.
	...
	A006519	knowna	7	Z.valueOf(ZUtils.valuation(v, Z.TWO))	Highest power of 2 dividing n.
digitprod	A007954	knowna	7	ZUtils.digitProduct(v)	Product of decimal digits of n.
...
usigma	A034448	knowna	7	Jaguar.factor(v).unitarySigma()	usigma(n) = sum of unitary divisors of n (divisors d such that gcd(d, n/d)=1); also called UnitarySigma(n).
digitsum	A053735	knowna	7	Z.valueOf(ZUtils.digitSum(v, 3))	Sum of digits of (n written in base 3).
...
square	A000290	knowna	7	v.square()		The squares: a(n) = n^2.
...
binary	A007088	knowna	6	new Z(Z.toString(2))		The binary numbers (or binary words, or binary vectors, or binary expansion of n): numbers written in base 2.
...
mersenne	A000225	knowna	5	Z.TWO.pow(v.intValueExact()).subtract(1)	a(n) = 2^n - 1. (Sometimes called Mersenne numbers, although that name is usually reserved for A001348.)
...
core	A007913	knowna	5	Jaguar.factor(v).core()	Squarefree part of n: a(n) is the smallest positive number m such that n/m is a square.
...
pod	A007955	knowna	1	Jaguar.factor(v).pod()	Product of divisors
moebius,mu,mobius,moebiusmu	A008683	knowna	1	Z.valueOf(Jaguar.factor(v).mobius())	Moebius mu function
sigma_2	A001157	knowna	4	Jaguar.factor(v).sigma(2)	
sigma_3	A001158	knowna	1	Jaguar.factor(v).sigma(3)	
