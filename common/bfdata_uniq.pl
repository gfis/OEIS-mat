...
# Keep only the longest line from 2 similar lines in bfdata.txt, and write the other to STDERR
# @(#) $Id$
# 2021-11-12, Georg Fischer
#
#:# Usage:
#:#   sort -k3,3 bfdata.txt \
#:#   perl bfdata_uniq.pl > bfdata_uniq.tmp 2> bfdata_prefix.tmp
#---------------------------------
use strict;
use integer;
use warnings;
    ...
    if ($olen >= $minlen) {
        if (0) {
            ...
            if (substr($ndata, 0, $olen) eq $odata) { # odata is prefix of ndata - skip it
                print STDERR join("\t", $oseqno, $occ      , $olen, $nseqno, $ncc) . "\n";
            } else { # start of new group - output odata
                print        join("\t", $oseqno, "bfunimax", $olen, $odata ) . "\n";
            }
        } else { #     == $olen
