...
#
#:# usage:
#:#   perl doubled_terms.pl stripped > outputfile
#---------------------------------
use strict;
use integer;
...
sub check_pairs {
    ...
    if ($busy == 1) {
        ...
        my $singlelist = join("\,", splice(@singles, 0, 16));
        my @asns = ();
        my $asnlist = "";
        my $asmin = "";
        my $skiplen = 0xfffffff; # very high
        if (length($singlelist) >= 16 && ! defined($ofters{$aseqno}) && ($distinct == 0 || $same == 0)) { # not yet in jOEIS
            my %strips = ();
            my %stlens = ();
            ...
            if (scalar(keys(%strips)) > 0) {
                if (length($prefix) == 0 && $skiplen == 0) { # no PrependSequence parameters
                    ...
                    print join("\t", $aseqno
                        ...
                        , substr($prefix, 1) # to be prefixed to target sequence
