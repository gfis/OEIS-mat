...
# Process MMA RecurrenceTable calls and extract parameters for HolonomicRecurrence
# @(#) $Id$
# 2021-06-16: shortened from rectab.pl
# 2020-02-11: show annihilator
# 2019-12-02, Georg Fischer
#
#:# Usage:
#:#   perl rectab.pl [-d] mmart3.tmp > holonomic.tmp
#---------------------------------
use strict;
use integer;
use warnings;
...
my ($left, $right);
my ($num, $den);
# print "# Generated by OEIS-mat/holrec/rectab.pl on $timestamp\n";
while (<>) {
    next if ! m{\AA\d};
    s{\s+\Z}{}; # chompr
    my ($aseqno, $callcode, $offset, $line, $ninits) = split(/\t/, $_);
    $line =~ s{\(\*.*}{};
    $line =~ s{\s}{}g;
    print "$aseqno\t# $line\n" if $debug >= 1;
    my $join = "";
    ...
    if ($line =~ m{\ARecurrenceTable\[\{([^\}]+)\}\,(\w+)(\[\w\])?\,\{([^\}]+)\}\]}) {
        my $exprlist = $1;
        my $variable = $2;
        my $range    = $4;
        ...
        if ($nyi == 3) {
            # print join("\t", $aseqno, "$callcode$nyi", $variable, $exprlist), "\n";
        }
        ...
        my $oldrec = $annihil;
        $oldrec =~ s{n\+0}{n}g;
        ($left, $right) = split(/\=/, $annihil);
        $right = $right || "0";
        if (defined($right)) {
            ($num, $den) = split(/\//, "$left$right");
            if (defined($den)) {
                $left .= "*$den";
            }
            $annihil = "-($left)+$right";
        ...
        $annihil =~ s{(\)|\d+|n)\(}{$1\*\(}g;
        #                0        1        2        3         4        5  6  7
        print join("\t", $aseqno, "holos", $offset, $annihil, $vector, 0, 0, $oldrec) . "\n";
