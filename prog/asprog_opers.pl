...
# Extract operators from asprog records (sequences of non-word, non-bracket, non-comma, non-space characters and their counts)
# @(#) $Id$
# 2026-07-15: Georg Fischer, copied from asprog_split.pl
#
#:# usage:
#:#   perl asprog_opers.pl [-d mode] asprog.txt
...
while (<>) {
    ...
    $program =~ s{\/\*[^\*\~]+\*\/}    {}g; # remove /* comment */
    $program =~ s{\(\*[^\*\~]+\*\/}    {}g; # remove (* comment *)
    $program =~ s{\\\"/}               {}g;
    $program =~ s{\\\'/}               {}g;
    $program =~ s{\\\`/}               {}g;
    $program =~ s{\"[^\"\~]*\"}        {}g; # remove strings with quotes
    $program =~ s{\'[^\'\~]*\'}        {}g; # remove strings with apostrophes
    $program =~ s{\`[^\`\~]*\`}        {}g; # remove strings with backticks
    $program =~ s{$prog_sep}         {A}g; # remove "~~"
    foreach my $oper ($program =~ m{([^a-zA-Z_0-9\,\# \(\)\[\]\{\}]+)}g) {
      if (defined($opers{$oper})) {
        $opers{$oper} ++;
      } else {
        $opers{$oper} = 1;
      }
...
foreach my $oper (keys(%opers)) {
  print "$oper\t$opers{$oper}\n";
} # foreach
#----
__DATA__
