...
#
#:# Usage:
#:#   perl ecalc.pl [-d debug[-b] ] [-m mode] [-n noterms] [-x] monster3.gp > etpsymm.gen
...
while (<>) {
    ...
    if (0) {
        ...
        #                                        parm1    parm2   parm3    parm4
        print join("\t", $aseqno, "etpsymm", -1, $factor, $add0,  $period, $orig, $chash{$code2} || "", $chash{$factor} || "") . "\n";
...
sub get_period {
    my ($vlist, $tlist) = @_;
    my @ts = ($tlist =~ m{(\-?\d+)}g); # ignore all separators
