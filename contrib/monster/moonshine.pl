...
my %bootcoefs = (
    "1A", (0, 196884, 21493760, 864299970, 20245856256, 333202640600),;
    ...
    "8E", (0, 4, 0, 2, 0, -8),
    "8F", (0, 0, 0, -6, 0, 0),
    "9A", (0, 27, 86, 243, 594, 1370), 
    "9B", (0, 0, 5, 0, 0, -7),
    "10A", (0, 22, 56, 177, 352, 870), 
    "10B", (0, 6, -8, 17, -32, 54),
    "10C", (0, -3, 6, 2, 2, -5), 
    "10D", (0, 21, 62, 162, 378, 819),
    "10E", (0, 1, 2, 2, -2, -1), 
    "11A", (0, 17, 46, 116, 252, 533),
    "12A", (0, 15, 32, 87, 192, 343), 
    "12B", (0, 6, -4, -3, 12, -8),
    "12C", (0, 7, 0, 15, 0, 71), 
    "12D", (0, 0, 8, 0, 0, 28),
    "12E", (0, -1, 0, 7, 0, -9), 
    "12F", (0, 6, 0, 21, 0, 56),
    "12G", (0, -2, 0, -3, 0, 8), 
    "12H", (0, 14, 36, 85, 180, 360),
    "12I", (0, 2, 0, 1, 0, 0), 
    "12J", (0, 0, 0, 0, 0, -4),
    "13A", (0, 12, 28, 66, 132, 258), 
    "13B", (0, -1, 2, 1, 2, -2),
    "14A", (0, 11, 20, 57, 92, 207), 
    "14B", (0, 3, -4, 9, -12, 15),
    "14C", (0, 10, 24, 51, 100, 190),
    "15A", (0, 8, 22, 42, 70, 155), 
    "15B", (0, -1, 4, -3, -2, 11),
    "15C", (0, 9, 19, 42, 78, 146), 
    "15D", (0, 0, -2, 0, 0, -1),
    "16A", (0, 4, 0, 10, 0, 24), 
    "16B", (0, 0, 0, 2, 0, 0),
    "16C", (0, 8, 16, 34, 64, 112), 
    "17A", (0, 7, 14, 29, 50, 92),
    "18A", (0, -2, 1, 0, 2, 1), 
    "18B", (0, 7, 10, 27, 38, 82),
    "18C", (0, 3, -2, 3, -6, 10), 
...
while (1) {
        ...
        my @jcoefs = ();
        foreach my $k (0..$lencomplete - 1) {
            push(@jcoefs, $coefs[$k]);
        }
        my $j = &powerSeries(0, @jcoefs);
        # Now compute the first Faber polynomials of j:
        my @jFaber = ("", $j);
        foreach $n (2, &min($lencomplete, 7) - 1) {
            ...
            my $jn = PowerSeries.multiply($jFaber[$n-1], $j);
            for (my $k = $n-2, $k >= 0; $k --) {
            ...
            my @ld = strictDivisors($n);
            foreach my $k (1..$jn.precis() - 1) {
    ...
    my $lastcomplete = $lencomplete;
    foreach my $k ($lenprinted..$lastcomplete - 1) {
        print sprintf("%d %d\n", $k + 1, $v);
    }
...
sub multiply { # Return a _new_ series by multiplying s1 and s2
    my ($s1, $s2) = @_;
    $index = s1.$index + s2.$index;
    my $length = &min(length(s1.$coefs), length(s2.$coefs));
...
sub addMonomial { # _Change_ a series by adding a monomial
    my ($c, $k) = @_;
    if (k < self.$index) {
...
sub addMonomialTimes { # Add a monomial times f
    my ($c, $k, $f) = @_;
    if ($k + $f.&precis() < self.$index) {
        #            raise Exception("PowerSeries defined nowhere")
    } elsif ($k + $f.precis() < self.precis()) { 
    ...
    foreach $i (0..length(self.$@coefs) - 1) {
...
sub coef { # Return a given coefficient
    my ($k) = @_;
    if ($k < self.$index) {
        return 0;
    } elsif ($k < self.precis()) { 
...
sub strictDivisors { # Return the list of divisors of n between 1 and n-1
    my ($n) = @_;
    my @result = ();
    foreach $i (1..n - 1) {
        if ($n % $i == 0) {
