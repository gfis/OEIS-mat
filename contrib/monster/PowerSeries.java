 ...
 */
public class PowerSeries extends ArrayList<BigInteger> {
  private int mIndex; 
  ...
  /**
   * Construct with index and {@link BigInteger} coefficients
   * @param index current index
   * @param coeffs coefficients of the power series
   */
  public PowerSeries(final int index, final ArrayList<BigInteger> coeffs) {
  	mIndex = index;
  ...
  public PowerSeries(final int index, final String list) {
  	...
  	for (int ic = 0; ic < len; ++ic) {
  	  super.add(new BigInteger(elems[ic]));
  	} // for ic
  }
  ...
  public PowerSeries multiply(final PowerSeries s2) {
  	...
  	ArrayList<BigInteger> coeffs = new ArrayList<BigInteger>(32);
  	for (int ic = 0; ic < len; ++ic) {
   ...
   * @param k exponent
   */
  public void addMonomial(final BigInteger c, final int k) {
    final int len = k;
    if (len < mIndex) {
      for (int ic = mIndex - len; ic > 0; --ic) {
        super.add(0, BigInteger.ZERO);
      }
      mIndex = len;
    }   
   ...
   */
  public void addMonomialTimes(final BigInteger c, final int k, final PowerSeries f) {
   ...
   */
  public BigInteger getCoeff(final int k) {
  ...
  public static void main(String[] args) {
    ...
    s3.addMonomial(BigInteger.valueOf(17),2);
    System.out.println(s3);
    s3.addMonomialTimes(BigInteger.valueOf(17),2,s1);
    System.out.println(s3);
  } // main
