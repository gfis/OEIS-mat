...
selected = sys.argv[1]
sDebug = 0
bfimax = 1000 # highest b-file index to be printed
if len(sys.argv) > 2:
    bfimax = int(sys.argv[2])
    sDebug = int(sys.argv[3])
    if sDebug >= 1:
        earlyprint = True
else:
    earlyprint = False
...
print("# OEIS b-file generated by moonshine4.py with \"" + sys.argv[1] +  "\" at " + datetime.now().strftime("%Y-%m-%d"))
    ...
    if not earlyprint:
        ...
        for k in range(lenprinted,lastcomplete):
            if (k > bfimax):
                ...
                for cl in selected[0:1]:
                    print (str(k) + " " + str(coefs[cl,k]))
